...
class PE_Dataset(pl.LightningDataModule):
    ...
    def _prepare_data(self):
        ...
        self.train_tuple = (train_initial_seq, train_mutated_seq, train_total_read_count, train_edited_percentage, train_unedited_percentage, train_indel_percentage, train_protospacer_location, train_pbs_location, train_rt_initial_location, train_rt_mutated_location)
        self.test_tuple = (test_initial_seq, test_mutated_seq, test_total_read_count, test_edited_percentage, test_unedited_percentage, test_indel_percentage, test_protospacer_location, test_pbs_location, test_rt_initial_location, test_rt_mutated_location)
    ...
    def _setup(self):
        ...
        train_initial_seq, train_mutated_seq, train_total_read_count, train_edited_percentage, train_unedited_percentage, train_indel_percentage, train_protospacer_location, train_pbs_location, train_rt_initial_location, train_rt_mutated_location = self.train_tuple
        test_initial_seq, test_mutated_seq, test_total_read_count, test_edited_percentage, test_unedited_percentage, test_indel_percentage, test_protospacer_location, test_pbs_location, test_rt_initial_location, test_rt_mutated_location = self.test_tuple

        train_initial_seq, val_initial_seq, train_mutated_seq, val_mutated_seq, train_total_read_count, val_total_read_count, train_edited_percentage, val_edited_percentage, train_unedited_percentage, val_unedited_percentage, train_indel_percentage, val_indel_percentage, train_protospacer_location, val_protospacer_location, train_pbs_location, val_pbs_location, train_rt_initial_location, val_rt_initial_location, train_rt_mutated_location, val_rt_mutated_location = train_test_split(train_initial_seq, train_mutated_seq, train_total_read_count, train_edited_percentage, train_unedited_percentage, train_indel_percentage, train_protospacer_location, train_pbs_location, train_rt_initial_location, train_rt_mutated_location, test_size = self.val_split, random_state = 42)
        
        pdb.set_trace()
        train_split_list = [torch.tensor(np.stack(train_initial_seq)).to(torch.int64), torch.tensor(np.stack(train_mutated_seq)).to(torch.int64), torch.tensor(np.stack(train_total_read_count)).to(torch.int64), torch.tensor(np.stack(train_edited_percentage)).to(torch.float32), torch.tensor(np.stack(train_unedited_percentage)).to(torch.float32), torch.tensor(np.stack(train_indel_percentage)).to(torch.float32), torch.tensor(np.stack(train_protospacer_location)).to(torch.int64), torch.tensor(np.stack(train_pbs_location)).to(torch.int64), torch.tensor(np.stack(train_rt_initial_location)).to(torch.int64), torch.tensor(np.stack(train_rt_mutated_location)).to(torch.int64)]
        val_split_list = [torch.tensor(np.stack(val_initial_seq)).to(torch.int64), torch.tensor(np.stack(val_mutated_seq)).to(torch.int64), torch.tensor(np.stack(val_total_read_count)).to(torch.int64), torch.tensor(np.stack(val_edited_percentage)).to(torch.float32), torch.tensor(np.stack(val_unedited_percentage)).to(torch.float32), torch.tensor(np.stack(val_indel_percentage)).to(torch.float32), torch.tensor(np.stack(val_protospacer_location)).to(torch.int64), torch.tensor(np.stack(val_pbs_location)).to(torch.int64), torch.tensor(np.stack(val_rt_initial_location)).to(torch.int64), torch.tensor(np.stack(val_rt_mutated_location)).to(torch.int64)]
        test_split_list = [torch.tensor(np.stack(test_initial_seq)).to(torch.int64), torch.tensor(np.stack(test_mutated_seq)).to(torch.int64), torch.tensor(np.stack(test_total_read_count)).to(torch.int64), torch.tensor(np.stack(test_edited_percentage)).to(torch.float32), torch.tensor(np.stack(test_unedited_percentage)).to(torch.float32), torch.tensor(np.stack(test_indel_percentage)).to(torch.float32), torch.tensor(np.stack(test_protospacer_location)).to(torch.int64), torch.tensor(np.stack(test_pbs_location)).to(torch.int64), torch.tensor(np.stack(test_rt_initial_location)).to(torch.int64), torch.tensor(np.stack(test_rt_mutated_location)).to(torch.int64)]
